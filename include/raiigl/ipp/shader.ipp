...
namespace raiigl {
  ...
  __forceinline std::string name( const shader_type& type )
  {
    ...
    else if( type == shader_type::Geometry ) return "Geometry";
    else return "Unknown";
  }
    ...
    id( glCreateShader( static_cast<GLuint>( type ) ) )
  {
    ...
    if( compile_status_result == GL_FALSE )
    {
      ...
      oss << "Error during '" << name( type ) << "' shader compilation : " << std::endl
